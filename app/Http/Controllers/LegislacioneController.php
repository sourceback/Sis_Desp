...
use App\Models\Legislacione as Modeloprincipal;
use App\Models\Archivolegislacione as Archivolegislacione;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\Session;
use Illuminate\Support\Str;

class LegislacioneController extends Controller
{
    //
    private $arrInfo = [
        'tabla' => 'legislaciones',
    ...
    private $arrRules = [
        'clave' => 'required|string|max:45|unique:legislaciones',
    ...
    public function archivolegislacione($id){

        $this->arrInfo['arrRecord'] = Modeloprincipal::find($id);
        return view($this->arrInfo['llave'].'/archivolegislacione', ['arrInfo' => $this->arrInfo]);
    }

    public function archivolegislacionedo(Request $request, $id){
        
        $file = $request->file('file');
        $path = public_path() . '/imageslegislaciones';
        $fileName = $file->getClientOriginalName().uniqid() ;
        $nombre = $file->getClientOriginalName();
        $file->move($path, $fileName);

        $projectImage = new Archivolegislacione();
        $projectImage->legislacione_id = $id;
        $projectImage->nombre = $nombre;
        $projectImage->archivo = '/images/legislaciones/'.$fileName;
        $projectImage->save();
    }

    public function download($id){

        $file = Archivolegislacione::where('id', $id)->firstOrFail();
    ...
    {
        $theRecord = Archivolegislacione::find($id);
