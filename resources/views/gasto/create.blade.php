          ...
          <form method="post" action="{{ route($arrInfo['ruta'].'.store') }}" >
            ...
            <div class="row">
              <div class="col-md-3">
                <div class="form-group">
                  <label>Monto</label>
                  <input type="text" class="form-control" max="9"  name="monto" value="{{ old('name', $arrInfo['arrRecord']['monto']) }}">
                  @error('monto')<small><strong style="color: red;">{{ $message }}</strong></small>@enderror
                </div>
              </div>
              <div class="col-md-4">
                <div class="form-group">
                  <label for="exampleInputEstatu1">Tipo de gasto</label>
                  <select class="form-control" name="tipo">
                      <option class="form-control" value="1" @if($arrInfo['arrRecord']['tipo']==1) selected="" @endif>Egreso</option>
                      <option class="form-control" value="0" @if($arrInfo['arrRecord']['tipo']==0) selected="" @endif>Ingreso</option>
                  </select>
                </div>
              </div>
              <div class="col-md-12">
